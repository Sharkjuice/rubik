...
class CubeController:
    def __init__(self, init_count, his_count):
        ...
        try:
            with open(".\\snapshots\\total", "r",1) as fo:
                for line in fo.readlines():
                    b = line.strip("(\n)").split(" ")
                    if b[0][0] != "#":
                        self.snapshots = int(b[1])
                        self.my_snapshot.setTotal(self.snapshots)
        except IOError as err:  
            fo = open(".\\snapshots\\total", "w", 1)
    ...
    def saveCube(self,flag):
        if flag == 0:
            ...
            if self.snapshots > 10:
                self.snapshots = 10
                fo = open(".\\mycube" + str(1) + ".clp", "w", 1)
            else:
                fo = open(".\\snapshots\\mycube" + str(self.snapshots) + ".clp", "w", 1)
                fs = open(".\\snapshots\\total", "w", 1)
                fs.write("#generated by this program, never modify it.\n")
                fs.write("total " + str(self.snapshots))
                fs.close() 
            self.my_snapshot.setTotal(self.snapshots)				
            self.message = u"保存为第" + str(self.snapshots) + "份魔方"    
