...
u"每次操作都是旋转90度。"],
    ...
    {"step":u"第一步", "title":u"底层十字还原",
     "text":[
u"找到白色中心块，定为底面, 然后把4个带",u"有白色面的棱块移至顶面，观察白色棱块侧",
u"面颜色,转动底下两层与对应中心块对齐(如",u"图1-1)将4个白色棱移至底面。这样完成一",
u"个底层十字，并且十字侧面颜色和中心块对",u"齐(如图1-3)。当白色块在顶层需要翻转时,",
u"(图1-2)转公式1: F R U"],
    "figures":[(0,0),(0,1),(0,2)]
    },    
    {"step":u"第二步", "title":u"底角归位(1)",
     "text":[
u"复原魔方第一层四个角会出现两种情况:  ",
u"1. 在顶层面找到一个有白色面的角块,转",u"动顶层，将角块放置前层右上角位置,观察",
u"角块另外两个颜色,转动魔方底下两层直到",u"到与前面和右面中心块颜色相对应(如图2-1,",
u"2-2, 2-3)。图2-1使用公式2-1。图2-2使",u"用公式2-2。图2-3使用公式2-3, 变成",
u"情况1，再使用公式2-1。",u"公式2-1：R U R' 公式2-2: F' U' F",
u"公式2-3为：RUU R'U'。" ],
     "figures":[(1,0),(1,1),(1,2)]
    },
    {"step":u"第二步", "title":u"底角归位(2)",
     "text":[
u"2. 如果底层没有复原，顶层也没有带白色",u"面的角块，观察底层找到位置错误的角块，",
u"放至前层右下角位置（如图2-4，2-5), 图",u"2-4使用公式2-4变成图2-1；图2-5使用公",
u"式2-5,变成图2-2。 再分别使用前述方法", u"就可完成。",
u"公式2-4：RUR'U'， 公式2-5：F' U' F U。"],
     "figures":[(1,3),(1,4)]
    },
    {"step":u"第三步", "title":u"中层棱块归位(1)",
     "text":[
u"首先需要在顶层找到一个没有黄色面的棱块，",u"转动上层，将棱块侧面颜色和中心颜色",
u"对齐,我们把对齐后的这面定义为右面，观察",u"棱块上面的颜色，此时会出现两种情况：",
u"1. 与前面中心块颜色一致(如图3-1)，此时",u"需要用公式3-1，棱块就能复位至前面位置。",
u"2. 如果与前面中心块颜色不一致(如图3-2)，",u"需要用公式3-2，就能复原到后层棱块位置。",
u"公式3-1：R'U'R'U'R' URUR(五逆四顺) ",u"公式3-2：RURUR U'R'U'R'(五顺四逆)"],
     "figures":[(2,0),(2,1)]
    },

    {"step":u"第三步", "title":u"中层棱块归位(2)",
     "text":[
u"3. 特殊情况：当中层出现棱块需要调整方",u"向时需要将棱块放于前层右棱块位置(如图",
u"3-3),转动公式3-1就能将棱块移至顶层，",u"再按照情况1或情况2的方法即可完成。",
u"公式3-1：R'U'R'U'R' URUR(五逆四顺) ",u"公式3-2：RURUR U 'R'U'R'(五顺四逆)"],
     "figures":[(2,2)]
    },
   ...
   {"step":u"第四步", "title":u"顶面架十字",
     "text":[
u"第三层的还原需要在顶面架一个黄色'十字'",u"架。在架十字的时候回出现三种情况。",
u"1.顶面已经有黄色块连成'一'字(如图4-1) ",u"这时需要转动公式4，就能加架好顶面黄色",
u"十字架。",
u"2.顶面已经有黄色棱块连成直角，把它定位",u"左上直角(如图4-2)，转动公式4，就变成情",
u"况1，按照情况1的方法就能将顶层十字架好。",
u"3.顶层只有一个黄色中心块(如图4-3)，这时",u"我们需要任意方向转动一次公式4就转变成情",
u"况2，再用情况2的方法就能将顶层十字架好。",u"公式4： F R U R' U' F'"],
     "figures":[(3,0),(3,1),(3,2)]
    },
   {"step":u"第五步", "title":u"顶角面归位(1)",
     "text":[
u"这一步我们需要将顶层黄色面归位，会出现",u"三种情况：",
u"1. 顶面只有一个黄色角块，转动顶面至右",u"前角位置(图5-1)，转动公式5即可完成。若",
u"没有复原，对好角块位置后，再重复一次公",u"式5-1，即可完成。",
u"公式5-1：R' U2 RU R' UR"],
     "figures":[(4,0)]
    },    
   {"step":u"第五步", "title":u"顶角面归位(2)",
     "text":[
u"2. 顶面有两个黄色的角块，转动顶层直到",u"右面的左上角出现黄色角块面(如图5-2、5-",
u"3、5-4),再转动公式5后变成情况1。按情况",u"1的方法即可完成。",
u"公式5-1：R' U2 RU R' UR"],
     "figures":[(4,1),(4,2),(4,3)]
    },
   {"step":u"第五步", "title":u"顶角面归位(3)",
     "text":[
u"3. 顶面没有黄色的角块，转动顶层直到前",u"面右上角位置出现黄色角块面(如图5-5)，",
u"转动公式5-1后变成情况1。",u"公式5-1：R' UU RU R' UR",
u"公式5-1本质是对角互换， 其中一角不变(",u"右面左上角），其它角原地顺时针旋转120",
u"后再换位。 因此观察，如果需要逆时针旋", u"可以用下面公式：",
...
u"不参与移动）"],
   ...
   {"step":u"第七步", "title":u"顶棱归位",
     "text":[
u"在顶棱归位时会出现两种情况：",
u"1. 顶层四个棱块中，有一个已经处于正确",u"位置, 我们把这一面定为左面(如图7-1)",
u"，转动公式7-1 一次或两次即可完成。",
u"2. 若顶层没有棱块在正确的位置(如图7-2)",u"转动公式7就会变成情况1。",
u"公式7：R2 U' f' U2 f U' R'2",          u"公式7-1本质上是三个棱逆时针互换。因此",
u"需要使用两次公式。这时，如果使用公式7-",u"2可以实现逆时针互换，就只需要使用一次",
u"公式7-2：F2 U r U2 r’ U F2"],
     "figures":[(6,0),(6,1)]
    }    
]
