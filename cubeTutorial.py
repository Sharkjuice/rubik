...
"text":[
u"找到白色中心块，定为底面(注意为观察方便",
u"把白色面放在上方了)。然后把4个带有白色面",
u"的棱块移至顶面，观察白色棱块侧面颜色,转",
u"动底层与对应中心块对齐(如图1-1),将4个白色",
u"棱移至底面。这样完成一个底层十字。注意侧",
u"面颜色和中心块要对齐(如图1-3)。当白色块需",
u"要翻转时(如图1-2)，使用公式1。",
u"",
u"公式1: F R U"],
"figures":[(0,0),(0,1),(0,2)]
...
#下面描述图例,每个()一个图例，()里面是F/R/U面
#F面从RUF角开始,从上往下，从右往左数9个小面的颜色
#R面从RUF角开始,从上往下，从左往右数9个小面的颜色
#U面从RUF角开始,从右往左，从左往右数9个小面的颜色
figList = [
